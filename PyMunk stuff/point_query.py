...
def on_mouse_press(x,y,button,modifiers):
    point_qry = space.point_query_nearest((x,y),0,pymunk.ShapeFilter()) #point_query_nearest returns point query object containing info about what shape is at max distance (2nd param) from point x,y (1st param)
    print(point_qry) #point query also has info about distance from shape to point (-ve) if point is inside shape, and gradient of this distance vector
    #point_query.shape,point_query.shape.body

    #segment query for ray casting
